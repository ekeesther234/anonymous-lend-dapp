...
import { loanStorage } from '../storage/loan';
...
export class LoanRequestController {
    ...
    async createLoanRequest(data) {
        if (!data.borrower || !data.amount || !data.interestRate || !data.duration) {
            return await RollupStateHandler.handleReport({
                error: 'Borrower, amount, interest rate, and duration must be provided.',
            });
        }
